...
def something_in_middle_v(coord,goal,positions):
    if(coord[1]< goal[1]):
        for i in range (0,len(positions)):
            for j in range(0,8):
                if(positions[i][j][0] == coord[0] and positions[i][j] != goal and positions[i][j][1]<goal[1]  and positions[i][j][1]>coord[1] and positions[i][j] != coord):
                    return True

    else:
        for i in range (0,len(positions)):
            for j in range(0,8):
                if(positions[i][j][0] == coord[0]  and positions[i][j] != goal and positions[i][j][1]<coord[1] and positions[i][j][1]>goal[1] and positions[i][j] != coord):
                    return True
    return False

def something_in_middle_h(coord,goal,positions):
    if(coord[0]<goal[0]): ##horizontally before
        for i in range (0,len(positions)):
            for j in range(0,8):
                if(positions[i][j][1] == coord[1] and positions[i][j] != goal and positions[i][j][0]<goal[0]  and positions[i][j][0]>coord[0] and positions[i][j] != coord):
                    return True

    else:
        for i in range (0,len(positions)):
            for j in range(0,8):
                if(positions[i][j][1] == coord[1]  and positions[i][j] != goal and positions[i][j][0]<coord[0] and positions[i][j][0]>goal[0] and positions[i][j] != coord):
                    return True
    return False
...
def get_dying_piece(pos,positions):
    ...
    for i in range (0,len(positions)):
        for j in range(0,8):
            if(positions[i][j] == coord):
                return (i,j)
    print("piece at : "+ str(pos) +", "+str(coord) + " not found" )
    return None
...
def get_piece_index(letter,but,color,positions):
    if(letter=='R'):
        if(positions[color][0] == (-1,-1) and positions[color][7] == (-1,-1)):
            return -1
        elif(positions[color][0] == (-1,-1)):
            return 7
        elif(positions[color][7] == (-1,-1)):
            return 0
        else:
            coord = positions[color][0]
        ...
        if(positions[color][1] == (-1,-1) and positions[color][6] == (-1,-1)):#no knight to move
            return -1
        else:
            if(positions[color][1] == (-1,-1)):#only one knight to move
                return 6
            elif (positions[color][6] == (-1,-1)):#only one knight to move
                return 1
            else:#decide which one can be moved here
                coord = positions[color][1]
                goal = transform_board_to_screen(but)
